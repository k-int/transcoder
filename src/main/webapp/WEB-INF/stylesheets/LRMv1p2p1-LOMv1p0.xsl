...
<xsl:transform version = "1.0" xmlns = "http://ltsc.ieee.org/xsd/LOM"
...
Brendon Towle, Thomson NETg, 16 September 2003
...
-->
	...
	<!-- ##MetaDataSchema -->
  <xsl:variable name = "metaDataSchemaHandling" select = "'fixed'"/>
<!--
  <xsl:variable name = "metaDataSchemaHandling" select = "'copy'"/>
-->

	<!-- ##UnknownSource -->
  <xsl:variable name = "unknownSource" select = "'unknown'"/>
...
-->
	...
	<xsl:if test="$namespace!='http://ltsc.ieee.org/xsd/LOM'
		and $namespace!='http://www.imsglobal.org/xsd/imsmd_rootv1p2p1'">
